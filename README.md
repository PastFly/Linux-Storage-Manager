...
M1A plans and M1B handoff/evidence/approval objects remain non-mutating. M1B1-M1B13 add lock/revalidation/journal/backup foundations, disposable recovery evidence, non-mutating backup capture/revalidation, immutable pre-mutation evidence, durable `PreconditionsVerified`, a durable exact `Approved` record, and a frozen non-executable semantic execution-intent manifest. There is no mutation-capable executor or apply command. Entering `Executing` remains separately gated on explicit owner acceptance and the remaining privileged execution/recovery controls described in the roadmap and safety documentation.

Implementation language: **Rust**.

> This repository is under active development. Current M0/M1A/M1B pre-executor code contains no storage-changing executor. Do not treat a preview, handoff, evidence bundle, `PreconditionsVerified` state, M1B12 `Approved` record, or M1B13 frozen intent manifest as authorization to modify production storage.
