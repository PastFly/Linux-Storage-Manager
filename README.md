...
The project has completed the read-only **M0 — Storage Discovery** and **M1A — Planning** baselines and is in **M1B pre-executor foundation work**. M1B0-M1B10 freeze the exact handoff, host lock, locked identity/capability revalidation, durable journal and verified backup/pre-mutation evidence. M1B11 durably verifies exact current-session evidence and advances only `IdentityRevalidated -> PreconditionsVerified`. M1B12 adds a durable exact-plan/evidence/target approval record and advances only `PreconditionsVerified -> Approved`. M1B13 freezes that exact approved `PlanStep` graph into a deterministic, typed, non-executable execution-intent manifest with a read-only verification barrier after every mutation candidate; the journal remains `Approved`. M1B14 then compiles the frozen intent into a typed non-executable native manifest with an explicit operation allowlist, preserved step dependencies, preserved verification barriers, deterministic digest binding and fail-closed native validation. M1B15 adds an exact non-executable `ResizePhysicalVolume` contract and promotes proven single-PV LVM underlying-capacity routes into exact chained previews for `partition? -> PV -> LV -> filesystem`, including native mutation-layer ordering checks. Storage mutation remains disabled. Storage discovery, Grow/Create planning, target identity guards and all current M1B APIs are still non-mutating: no current CLI/TUI path changes disks, partitions, LVM metadata, filesystems, mounts, or swap configuration.
...
M1A plans and M1B handoff/evidence/approval objects remain non-mutating. M1B1-M1B15 add lock/revalidation/journal/backup foundations, disposable recovery evidence, non-mutating backup capture/revalidation, immutable pre-mutation evidence, durable `PreconditionsVerified`, a durable exact `Approved` record, a frozen non-executable semantic execution-intent manifest, a typed native manifest compiler/validator, and exact chained LVM growth previews including PV resize intent. There is no mutation-capable executor or apply command. Entering `Executing` remains separately gated on explicit owner acceptance and the remaining privileged execution/recovery controls described in the roadmap and safety documentation.

Implementation language: **Rust**.

> This repository is under active development. Current M0/M1A/M1B pre-executor code contains no storage-changing executor. Do not treat a preview, handoff, evidence bundle, `PreconditionsVerified` state, M1B12 `Approved` record, M1B13 frozen intent manifest, M1B14 native manifest, or M1B15 chained preview as authorization to modify production storage.
