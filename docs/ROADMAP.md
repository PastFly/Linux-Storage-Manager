...
- [x] Add an opt-in production-mutation compile boundary with a root-only, explicit-confirmation, one-step arm bound to the exact durable journal/request/permit/target/rolling identity; the armed executor must revalidate the current durable journal immediately before descriptor execution while the normal build remains read-only.
