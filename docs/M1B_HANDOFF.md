...
M1B35 introduces an explicit production arming contract without changing the default read-only build. The new `production-mutation` compile feature only exposes an armed descriptor-exec entry point; it does not flip `MUTATION_ENABLED`. Each arm is root-only, requires an exact destructive-action confirmation phrase, and is bound to the current durable journal digest, execution, request, launch permit, plan step, target/device and rolling target-identity digest. Immediately before fexecve, the armed executor re-loads the durable journal under the existing host lock and must reproduce the exact same arm. A stale journal, step, permit, request, target or identity therefore fails closed. No normal CLI/app path enables this feature yet.
